...
#define IM_STATIC_VERIFY(_COND)         static_assert(_COND, "")
#else
#define IM_STATIC_VERIFY(_COND)         typedef char static_assertion_##__line__[(_COND)?1:-1]
#endif
...
#define IM_VERIFY_PARANOID(_EXPR)       IM_VERIFY(_EXPR)
#else
#define IM_VERIFY_PARANOID(_EXPR)
#endif

// Error handling
// Down the line in some frameworks/languages we would like to have a way to redirect those to the programmer and recover from more faults.
#ifndef IM_VERIFY_USER_ERROR
#define IM_VERIFY_USER_ERROR(_EXP,_MSG) IM_VERIFY((_EXP) && _MSG)   // Recoverable User Error
#endif
...
#else
#define IM_DEBUG_BREAK()    IM_VERIFY(0)    // It is expected that you define IM_DEBUG_BREAK() into something that will break nicely in a debugger!
#endif
...
struct IMGUI_API ImBitArray
{
    ...
    bool            TestBit(int n) const        { IM_VERIFY(n < BITCOUNT); return ImBitArrayTestBit(Storage, n); }
    void            SetBit(int n)               { IM_VERIFY(n < BITCOUNT); ImBitArraySetBit(Storage, n); }
    void            ClearBit(int n)             { IM_VERIFY(n < BITCOUNT); ImBitArrayClearBit(Storage, n); }
...
struct IMGUI_API ImBitVector
{
    ...
    bool            TestBit(int n) const        { IM_VERIFY(n < (Storage.Size << 5)); return ImBitArrayTestBit(Storage.Data, n); }
    void            SetBit(int n)               { IM_VERIFY(n < (Storage.Size << 5)); ImBitArraySetBit(Storage.Data, n); }
    void            ClearBit(int n)             { IM_VERIFY(n < (Storage.Size << 5)); ImBitArrayClearBit(Storage.Data, n); }
};
...
struct ImSpan
{
    ...
    inline T&           operator[](int i)           { T* p = Data + i; IM_VERIFY(p >= Data && p < DataEnd); return *p; }
    inline const T&     operator[](int i) const     { const T* p = Data + i; IM_VERIFY(p >= Data && p < DataEnd); return *p; }
    ...
    // Utilities
    inline int  index_from_ptr(const T* it) const   { IM_VERIFY(it >= Data && it < DataEnd); const ptrdiff_t off = it - Data; return (int)off; }
};
...
struct ImSpanAllocator
{
    ...
    inline void  Reserve(int n, size_t sz, int a=4) { IM_VERIFY(n == CurrIdx && n < CHUNKS); CurrOff = IM_MEMALIGN(CurrOff, a); Offsets[n] = CurrOff; Sizes[n] = (int)sz; CurrIdx++; CurrOff += (int)sz; }
    inline int   GetArenaSizeInBytes()              { return CurrOff; }
    inline void  SetArenaBasePtr(void* base_ptr)    { BasePtr = (char*)base_ptr; }
    inline void* GetSpanPtrBegin(int n)             { IM_VERIFY(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (void*)(BasePtr + Offsets[n]); }
    inline void* GetSpanPtrEnd(int n)               { IM_VERIFY(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (void*)(BasePtr + Offsets[n] + Sizes[n]); }
    template<typename T>
...
struct IMGUI_API ImPool
{
    ...
    ImPoolIdx   GetIndex(const T* p) const          { IM_VERIFY(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
...
struct IMGUI_API ImChunkStream
{
    ...
    T*      next_chunk(T* p)            { size_t HDR_SZ = 4; IM_VERIFY(p >= begin() && p < end()); p = (T*)(void*)((char*)(void*)p + chunk_size(p)); if (p == (T*)(void*)((char*)end() + HDR_SZ)) return (T*)0; IM_VERIFY(p < end()); return p; }
    int     chunk_size(const T* p)      { return ((const int*)p)[-1]; }
    T*      end()                       { return (T*)(void*)(Buf.Data + Buf.Size); }
    int     offset_from_ptr(const T* p) { IM_VERIFY(p >= begin() && p < end()); const ptrdiff_t off = (const char*)p - Buf.Data; return (int)off; }
    T*      ptr_from_offset(int off)    { IM_VERIFY(off >= 4 && off < Buf.Size); return (T*)(void*)(Buf.Data + off); }
...
struct ImGuiTabBar
{
    ...
    {
        IM_VERIFY(tab->NameOffset != -1 && tab->NameOffset < TabsNames.Buf.Size);
...
namespace ImGui
{
    ...
    inline bool FocusableItemRegister(ImGuiWindow* window, ImGuiID id)  { IM_VERIFY(0); IM_UNUSED(window); IM_UNUSED(id); return false; } // -> pass ImGuiItemAddFlags_Focusable flag to ItemAdd()
    inline void FocusableItemUnregister(ImGuiWindow* window)            { IM_VERIFY(0); IM_UNUSED(window); }                              // -> unnecessary: TempInputText() uses ImGuiInputTextFlags_MergedItem
#endif
    ...
    inline bool             IsActiveIdUsingKey(ImGuiKey key)                            { ImGuiContext& g = *GImGui; IM_VERIFY(key < 64); return (g.ActiveIdUsingKeyInputMask & ((ImU64)1 << key)) != 0; }
