...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Sanity checks
    IM_VERIFY(columns_count > 0 && columns_count <= IMGUI_TABLE_MAX_COLUMNS && "Only 1..64 columns allowed!");
    if (flags & ImGuiTableFlags_ScrollX)
        IM_VERIFY(inner_width >= 0.0f);
    ...
    if (instance_no > 0)
        IM_VERIFY(table->ColumnsCount == columns_count && "BeginTable(): Cannot change columns count mid-frame while preserving same ID");
    ...
    if (use_child_window)
    {
        ...
        IM_VERIFY(table->InnerWindow->WindowPadding.x == 0.0f && table->InnerWindow->WindowPadding.y == 0.0f && table->InnerWindow->WindowBorderSize == 0.0f);
    }
    else
    {
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            IM_VERIFY(reorder_dir == -1 || reorder_dir == +1);
            IM_VERIFY(table->Flags & ImGuiTableFlags_Reorderable);
            ...
            IM_VERIFY(dst_column->DisplayOrder == dst_order - reorder_dir);
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in)
{
    ...
    {
        IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of each set is used.
    }
    ...
    //IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_AlignMask_)); // Check that only 1 of each set is used.
...
{
    ImGuiContext& g = *GImGui;
    IM_VERIFY(table->IsLayoutLocked == false);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        IM_VERIFY(column->IndexWithinEnabledSet <= column->DisplayOrder);
    ...
    IM_VERIFY(table->LeftMostEnabledColumn >= 0 && table->RightMostEnabledColumn >= 0);
...
{
    ImGuiContext& g = *GImGui;
    IM_VERIFY(table->Flags & ImGuiTableFlags_Resizable);
...
void    ImGui::EndTable()
{
    ...
    IM_VERIFY(table != NULL && "Only call EndTable() if BeginTable() returns true!");

    // This assert would be very useful to catch a common error... unfortunately it would probably trigger in some
    // cases, and for consistency user may sometimes output empty tables (and still benefit from e.g. outer border)
    //IM_VERIFY(table->IsLayoutLocked && "Table unused: never called TableNextRow(), is that the intent?");
    ...
    IM_VERIFY(inner_window == g.CurrentWindow);
    IM_VERIFY(outer_window == inner_window || outer_window == inner_window->ParentWindow);
    ...
    IM_VERIFY(table->RowPosY2 == inner_window->DC.CursorPos.y);
    ...
    // Pop from id stack
    IM_VERIFY_USER_ERROR(inner_window->IDStack.back() == table->ID + table->InstanceCurrent, "Mismatching PushID/PopID!");
    IM_VERIFY_USER_ERROR(outer_window->DC.ItemWidthStack.Size >= temp_data->HostBackupItemWidthStackSize, "Too many PopItemWidth!");
    PopID();
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
        ...
        IM_VERIFY((table->Flags & ImGuiTableFlags_ScrollX) == 0);
    ...
    IM_VERIFY(g.CurrentWindow == outer_window && g.CurrentTable == table);
    IM_VERIFY(g.CurrentTableStackIdx >= 0);
    g.CurrentTableStackIdx--;
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL && "Need to call TableSetupColumn() after BeginTable()!");
    IM_VERIFY(table->IsLayoutLocked == false && "Need to call call TableSetupColumn() before first row!");
    IM_VERIFY((flags & ImGuiTableColumnFlags_StatusMask_) == 0 && "Illegal to pass StatusMask values to TableSetupColumn()");
    if (table->DeclColumnsCount >= table->ColumnsCount)
    {
        IM_VERIFY_USER_ERROR(table->DeclColumnsCount < table->ColumnsCount, "Called TableSetupColumn() too many times!");
        return;
    }
    ...
    if (table->IsDefaultSizingPolicy && (flags & ImGuiTableColumnFlags_WidthMask_) == 0 && (flags & ImGuiTableFlags_ScrollX) == 0)
        IM_VERIFY(init_width_or_weight <= 0.0f && "Can only specify width/weight if sizing policy is set explicitly in either Table or Column.");
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL && "Need to call TableSetupColumn() after BeginTable()!");
    IM_VERIFY(table->IsLayoutLocked == false && "Need to call TableSetupColumn() before first row!");
    IM_VERIFY(columns >= 0 && columns < IMGUI_TABLE_MAX_COLUMNS);
    IM_VERIFY(rows >= 0 && rows < 128); // Arbitrary limit
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL);
    if (!table)
        return;
    IM_VERIFY(table->Flags & ImGuiTableFlags_Hideable); // See comments above
    if (column_n < 0)
        column_n = table->CurrentColumn;
    IM_VERIFY(column_n >= 0 && column_n < table->ColumnsCount);
    ImGuiTableColumn* column = &table->Columns[column_n];
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(target != ImGuiTableBgTarget_None);
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    IM_VERIFY(window == table->InnerWindow);
    IM_VERIFY(table->IsInsideRow);
    ...
    {
        IM_VERIFY(table->IsUnfrozenRows == false);
        table->IsUnfrozenRows = true;
        ...
        IM_VERIFY(table->Bg2ClipRectForDrawCmd.Min.y <= table->Bg2ClipRectForDrawCmd.Max.y);
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    if (table->CurrentColumn != column_n)
    {
        if (table->CurrentColumn != -1)
            TableEndCell(table);
        IM_VERIFY(column_n >= 0 && table->ColumnsCount);
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_NoClip)
    {
        ...
        //IM_VERIFY(table->DrawSplitter._Current == TABLE_DRAW_CHANNEL_NOCLIP);
    }
    else
    {
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL && table->IsLayoutLocked == false);
    IM_VERIFY(column_n >= 0 && column_n < table->ColumnsCount);
    ...
    IM_VERIFY(table->MinColumnWidth > 0.0f);
    ...
    IM_VERIFY(column_0_width > 0.0f && column_1_width > 0.0f);
    column_0->WidthRequest = column_0_width;
...
{
    IM_VERIFY(table->LeftMostStretchedColumn != -1 && table->RightMostStretchedColumn != -1);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (!column->IsEnabled || !(column->Flags & ImGuiTableColumnFlags_WidthStretch))
            continue;
        IM_VERIFY(column->StretchWeight > 0.0f);
        visible_weight += column->StretchWeight;
        visible_width += column->WidthRequest;
    }
    IM_VERIFY(visible_weight > 0.0f && visible_width > 0.0f);
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    IM_VERIFY(table->BgClipRect.Min.y <= table->BgClipRect.Max.y);
}
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            IM_VERIFY(channel_no < IMGUI_TABLE_MAX_DRAW_CHANNELS);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_VERIFY(has_freeze_v == false || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (int n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
                    ...
                    IM_VERIFY(channel->_CmdBuffer.Size == 1 && merge_clip_rect.Contains(ImRect(channel->_CmdBuffer[0].ClipRect)));
        ...
        IM_VERIFY(dst_tmp == g.DrawChannelsTempMergeBuffer.Data + g.DrawChannelsTempMergeBuffer.Size);
...
{
    IM_VERIFY(n < column->SortDirectionsAvailCount);
...
IM_STATIC_VERIFY(ImGuiSortDirection_None == 0 && ImGuiSortDirection_Ascending == 1 && ImGuiSortDirection_Descending == 2);
ImGuiSortDirection ImGui::TableGetColumnNextSortDirection(ImGuiTableColumn* column)
{
    IM_VERIFY(column->SortDirectionsAvailCount > 0);
    if (column->SortOrder == -1)
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs)
{
    ...
    if (!(table->Flags & ImGuiTableFlags_SortTristate))
        IM_VERIFY(sort_direction != ImGuiSortDirection_None);
...
{
    IM_VERIFY(table->Flags & ImGuiTableFlags_Sortable);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        IM_VERIFY(sort_order_count < (int)sizeof(sort_order_mask) * 8);
    }
    ...
    if (need_fix_linearize || need_fix_single_sort_order)
    {
        ...
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            ...
            IM_VERIFY(column_with_smallest_sort_order != -1);
...
void ImGui::TableSortSpecsBuild(ImGuiTable* table)
{
    ...
    if (dirty && sort_specs != NULL)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ...
            if (column->SortOrder == -1)
                continue;
            IM_VERIFY(column->SortOrder < table->SortSpecsCount);
...
void ImGui::TableHeadersRow()
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL && "Need to call TableHeadersRow() after BeginTable()!");
...
void ImGui::TableHeader(const char* label)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_VERIFY(table != NULL && "Need to call TableHeader() after BeginTable()!");
    IM_VERIFY(table->CurrentColumn != -1);
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ...
    if (column_n == table->ColumnsCount)                // To facilitate using with TableGetHoveredColumn()
        column_n = -1;
    IM_VERIFY(column_n >= -1 && column_n < table->ColumnsCount);
...
ImGuiTableSettings* ImGui::TableGetBoundSettings(ImGuiTable* table)
{
    if (table->SettingsOffset != -1)
    {
        ...
        IM_VERIFY(settings->ID == table->ID);
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    IM_VERIFY(settings->ID == table->ID);
    IM_VERIFY(settings->ColumnsCount == table->ColumnsCount && settings->ColumnsCountMax >= settings->ColumnsCount);
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    ImGuiContext& g = *GImGui;
    IM_VERIFY(table->MemoryCompacted == false);
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_VERIFY(column_index > 0); // We are not supposed to drag column 0.
    IM_VERIFY(g.ActiveId == columns->ID + ImGuiID(column_index));
...
float ImGui::GetColumnOffset(int column_index)
{
    ...
    IM_VERIFY(column_index < columns->Columns.Size);
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    IM_VERIFY(columns_count >= 1);
    IM_VERIFY(window->DC.CurrentColumns == NULL);   // Nested columns are currently not supported

    // Acquire storage for the columns set
    ImGuiID id = GetColumnsID(str_id, columns_count);
    ImGuiOldColumns* columns = FindOrCreateColumns(window, id);
    IM_VERIFY(columns->ID == id);
    columns->Current = 0;
