...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            IM_VERIFY(mouse_button >= 0 && mouse_button < ImGuiMouseButton_COUNT);
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    IM_VERIFY(size_arg.x != 0.0f && size_arg.y != 0.0f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    IM_VERIFY(ImMax(size_contents_v, size_avail_v) > 0.0f); // Adding this assert to check if the ImMax(XXX,1.0f) is still needed. PLEASE CONTACT ME if this triggers.
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    IM_VERIFY(ImIsPowerOfTwo(flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)));   // Check that only 1 option is selected
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ...
    if (held)
    {
        ...
        {
            if (mouse_delta < 0.0f)
                IM_VERIFY(*size1 + mouse_delta >= min_size1);
            if (mouse_delta > 0.0f)
                IM_VERIFY(*size2 - mouse_delta >= min_size2);
            *size1 += mouse_delta;
            *size2 -= mouse_delta;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    IM_VERIFY((flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can't use both flags together
    ...
    if (flags & ImGuiComboFlags_CustomPreview)
    {
        ...
        IM_VERIFY(preview_value == NULL || preview_value[0] == 0);
        preview_value = NULL;
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    else
    {
        ...
        IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiComboFlags_HeightMask_)); // Only one
    ...
    {
        EndPopup();
        IM_VERIFY(0);   // This should never happen as we tested for IsPopupOpen() above
        return false;
    }
    return true;
}
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems || !window->ClipRect.Overlaps(window->DC.LastItemRect)) // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible flag to test last ItemAdd() result
        return false;
    IM_VERIFY(window->DC.LastItemRect.Min.x == preview_data->PreviewRect.Min.x && window->DC.LastItemRect.Min.y == preview_data->PreviewRect.Min.y); // Didn't call after BeginCombo/EndCombo block or forgot to pass ImGuiComboFlags_CustomPreview flag?
...
IM_STATIC_VERIFY(IM_ARRAYSIZE(GDataTypeInfo) == ImGuiDataType_COUNT);
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        return g.TempBuffer;
#else
        IM_VERIFY(0 && "DragInt(): Invalid format string!"); // Old versions used a default parameter of "%.0f", please replace with e.g. "%d"
#endif
    }
    return fmt;
}
...
{
    IM_VERIFY(data_type >= 0 && data_type < ImGuiDataType_COUNT);
...
static void SanitizeFormatString(const char* fmt, char* fmt_out, size_t fmt_out_size)
{
    ...
    IM_VERIFY((size_t)(fmt_end - fmt + 1) < fmt_out_size); // BitmapFormat is too long, let us know if this happens to you!
    while (fmt < fmt_end)
    {
        char c = *(fmt++);
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
    ...
    // BitmapFormat value with our rounding, and read back
    char v_str[64];
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_VERIFY((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flags! Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (power != 1.0f)
    {
        IM_VERIFY(power == 1.0f && "Call function with ImGuiSliderFlags_Logarithmic flags instead of using the old 'float power' function!");
        IM_VERIFY(p_min != NULL && p_max != NULL);  // When using a power curve the drag needs to have known bounds
...
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_VERIFY((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
    ...
    case ImGuiDataType_S32:
        IM_VERIFY(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
        return SliderBehaviorT<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v,  *(const ImS32*)p_min,  *(const ImS32*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_U32:
        IM_VERIFY(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v,  *(const ImU32*)p_min,  *(const ImU32*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_S64:
        IM_VERIFY(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v,  *(const ImS64*)p_min,  *(const ImS64*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_U64:
        IM_VERIFY(*(const ImU64*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v,  *(const ImU64*)p_min,  *(const ImU64*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_Float:
        IM_VERIFY(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorT<float, float, float >(bb, id, data_type, (float*)p_v,  *(const float*)p_min,  *(const float*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_Double:
        IM_VERIFY(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
...
{
    IM_VERIFY(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* new_text, int new_text_len)
{
    ...
    if (new_text_len + text_len + 1 > obj->TextW.Size)
    {
        if (!is_resizable)
            return false;
        IM_VERIFY(text_len < obj->TextW.Size);
...
static void stb_textedit_replace(ImGuiInputTextState* str, STB_TexteditState* state, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
    ...
    IM_VERIFY(0); // Failed to insert character, normally shouldn't happen because of how we currently use stb_textedit_replace()
}
...
{
    IM_VERIFY(pos + bytes_count <= BufTextLen);
    char* dst = Buf + pos;
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize)
    {
        ...
        IM_VERIFY(edit_state->ID != 0 && g.ActiveId == edit_state->ID);
        IM_VERIFY(Buf == edit_state->TextA.Data);
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    IM_VERIFY(input_source == ImGuiInputSource_Keyboard || input_source == ImGuiInputSource_Clipboard);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    IM_VERIFY(buf != NULL && buf_size >= 0);
    IM_VERIFY(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline)));        // Can't use both together (they both use up/down keys)
    IM_VERIFY(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together (they both use tab key)
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    if (is_resizable)
        IM_VERIFY(callback != NULL); // Must provide a callback if you set the ImGuiInputTextFlags_CallbackResize flag!
    ...
    {
        IM_VERIFY(state && state->ID == id);
        SetActiveID(id, window);
        SetFocusID(id, window);
        FocusWindow(window);

        // Declare our inputs
        IM_VERIFY(ImGuiNavInput_COUNT < 32);
        g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
    ...
    if (is_password && !is_displaying_hint)
    {
        ...
        //IM_VERIFY(password_font->Glyphs.empty() && password_font->IndexAdvanceX.empty() && password_font->IndexLookup.empty());
    ...
    {
        IM_VERIFY(state != NULL);
        IM_VERIFY(io.KeyMods == GetMergedKeyModFlags() && "Mismatching io.KeyCtrl/io.KeyShift/io.KeyAlt/io.KeySuper vs io.KeyMods"); // We rarely do this check, but if anything let's do it here.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    IM_VERIFY(callback_data.Buf == state->TextA.Data);  // Invalid to modify those fields
                    IM_VERIFY(callback_data.BufSize == state->BufCapacityA);
                    IM_VERIFY(callback_data.Flags == flags);
                    ...
                    if (buf_dirty)
                    {
                        IM_VERIFY(callback_data.BufTextLen == (int)strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
        ...
        if (apply_new_text)
        {
            ...
            IM_VERIFY(apply_new_text_length >= 0);
            if (is_resizable)
            {
                ImGuiInputTextCallbackData callback_data;
                ...
                IM_VERIFY(apply_new_text_length <= buf_size);
            }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DisplayMask)); // Check that only 1 is selected
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__InputMask));   // Check that only 1 is selected
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__PickerMask)); // Check that only 1 is selected
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__InputMask));  // Check that only 1 is selected
    if (!(flags & ImGuiColorEditFlags_NoOptions))
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
    ...
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DisplayMask));    // Check only 1 option is selected
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DataTypeMask));   // Check only 1 option is selected
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__PickerMask));     // Check only 1 option is selected
    IM_VERIFY(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__InputMask));      // Check only 1 option is selected
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (!is_leaf)
    {
        ...
        {
            IM_VERIFY(button_flags & ImGuiButtonFlags_PressedOnDragDropHold);
...
void ImGui::TreePop()
{
    ...
    IM_VERIFY(window->IDStack.Size > 1); // There should always be 1 element in the IDStack (pushed during window creation). If this triggers you called TreePop/PopID too much.
    PopID();
}
...
void ImGui::EndListBox()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_VERIFY((window->Flags & ImGuiWindowFlags_ChildWindow) && "Mismatched BeginListBox/EndListBox calls. Did you test the return value of BeginListBox?");
    IM_UNUSED(window);
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            IM_VERIFY(v_idx >= 0 && v_idx < values_count);
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            IM_VERIFY(v1_idx >= 0 && v1_idx < values_count);
...
bool ImGui::BeginMenuBar()
{
    ...
    IM_VERIFY(!window->DC.MenuBarAppending);
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            IM_VERIFY(window->DC.NavLayersActiveMaskNext & (1 << layer)); // Sanity check
            FocusWindow(window);
    ...
    IM_VERIFY(window->Flags & ImGuiWindowFlags_MenuBar);
    IM_VERIFY(window->DC.MenuBarAppending);
    PopClipRect();
    PopID();
...
void    ImGui::EndTabBar()
{
    ...
    if (tab_bar == NULL)
    {
        IM_VERIFY_USER_ERROR(tab_bar != NULL, "Mismatched BeginTabBar()/EndTabBar()!");
        return;
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        IM_VERIFY(tab->LastFrameVisible >= tab_bar->PrevFrameVisible);
        ...
        IM_VERIFY(tab->ContentWidth > 0.0f);
...
{
    IM_VERIFY(!(tab->Flags & ImGuiTabItemFlags_Button));
...
{
    IM_VERIFY(offset != 0);
    IM_VERIFY(tab_bar->ReorderRequestTabId == 0);
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    //IM_VERIFY(tab_bar->Flags & ImGuiTabBarFlags_Reorderable); // <- this may happen when using debug tools
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (tab_bar == NULL)
    {
        IM_VERIFY_USER_ERROR(tab_bar, "Needs to be called between BeginTabBar() and EndTabBar()!");
        return false;
    }
    IM_VERIFY(!(flags & ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be used with button flags, use TabItemButton() instead!
...
void    ImGui::EndTabItem()
{
    ...
    if (tab_bar == NULL)
    {
        IM_VERIFY_USER_ERROR(tab_bar != NULL, "Needs to be called between BeginTabBar() and EndTabBar()!");
        return;
    }
    IM_VERIFY(tab_bar->LastTabItemIdx >= 0);
    ImGuiTabItem* tab = &tab_bar->Tabs[tab_bar->LastTabItemIdx];
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    IM_VERIFY(!p_open || !(flags & ImGuiTabItemFlags_Button));
    IM_VERIFY((flags & (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) != (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)); // Can't use both Leading and Trailing
    ...
    IM_VERIFY(!is_tab_button || !(tab_bar->SelectedTabId == tab->ID && is_tab_button)); // TabItemButton should not be selected
    if (is_tab_button)
        return pressed;
    return tab_contents_visible;
}
