...
#define STBRP_STATIC
#define STBRP_VERIFY(x)     do { IM_VERIFY(x); } while (0)
...
#define STBTT_assert(x)     do { IM_VERIFY(x); } while(0)
...
void ImDrawList::_ResetForNewFrame()
{
    ...
    // (those should be IM_STATIC_VERIFY() in theory but with our pre C++11 setup the whole check doesn't compile with GCC)
    IM_VERIFY(IM_OFFSETOF(ImDrawCmd, ClipRect) == 0);
    IM_VERIFY(IM_OFFSETOF(ImDrawCmd, TextureId) == sizeof(ImVec4));
    IM_VERIFY(IM_OFFSETOF(ImDrawCmd, VtxOffset) == sizeof(ImVec4) + sizeof(ImTextureID));
...
void ImDrawList::AddDrawCmd()
{
    ...
    IM_VERIFY(draw_cmd.ClipRect.x <= draw_cmd.ClipRect.z && draw_cmd.ClipRect.y <= draw_cmd.ClipRect.w);
...
{
    ImDrawCmd* curr_cmd = &CmdBuffer.Data[CmdBuffer.Size - 1];
    IM_VERIFY(curr_cmd->UserCallback == NULL);
...
void ImDrawList::_OnChangedVtxOffset()
{
    ...
    _VtxCurrentIdx = 0;
    ImDrawCmd* curr_cmd = &CmdBuffer.Data[CmdBuffer.Size - 1];
    //IM_VERIFY(curr_cmd->VtxOffset != _CmdHeader.VtxOffset); // See #3349
    if (curr_cmd->ElemCount != 0)
    {
        AddDrawCmd();
        return;
...
void ImDrawList::PrimReserve(int idx_count, int vtx_count)
{
    ...
    IM_VERIFY_PARANOID(idx_count >= 0 && vtx_count >= 0);
...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count, ImU32 col, ImDrawFlags flags, float thickness)
{
    ...
    if (Flags & ImDrawListFlags_AntiAliasedLines)
    {
        ...
        IM_VERIFY_PARANOID(!use_texture || !(_Data->Font->ContainerAtlas->Flags & ImFontAtlasFlags_NoBakedLines));
...
void ImDrawList::_PathArcToFastEx(const ImVec2& center, float radius, int a_min_sample, int a_max_sample, int a_step)
{
    ...
    IM_VERIFY_PARANOID(_Path.Data + _Path.Size == out_ptr);
}
...
IM_STATIC_VERIFY(ImDrawFlags_RoundCornersTopLeft == (1 << 4));
static inline ImDrawFlags FixRectCornerFlags(ImDrawFlags flags)
{
    ...
    IM_VERIFY((flags & 0x0F) == 0 && "Misuse of legacy hardcoded ImDrawCornerFlags values!");
...
{
    IM_UNUSED(draw_list);
    IM_VERIFY(_Current == 0 && _Count <= 1 && "Nested channel splitting is not supported. Please use separate instances of ImDrawListSplitter.");
...
ImFontAtlas::ImFontAtlas(int tex_width, int tex_height)
{
    ...
    IM_VERIFY(OverflowTexWidth < TexWidth);
    IM_VERIFY(OverflowTexHeight < TexHeight);
...
void ImFontAtlas::PrepareFontTexture() {
    ...
    //Render mouse cursors
    {
        ...
        IM_VERIFY(r->FontTexture->TexID == FontTextures.front()->TexID);
    ...
    if (!(this->Flags & ImFontAtlasFlags_NoBakedLines)) {
        ...
        for (unsigned int n = 0; n < IM_DRAWLIST_TEX_LINES_WIDTH_MAX + 1; n++) // +1 because of the zero-width row
        {
            ...
            // Write each slice
            IM_VERIFY(pad_left + line_width + pad_right == r->Width && y < r->Height); // Make sure we're inside the texture bounds before we start writing pixels
    ...
    if(!(this->Flags & ImFontAtlasFlags_NoMouseCursors)) {
        ...
        IM_VERIFY(FontTextures.Size);
        IM_VERIFY(FontTextures.front()->FontTexRows.Size);
...
{
//    IM_VERIFY(!Locked && "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!");
    Clear();
...
{
 //   IM_VERIFY(!Locked && "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!");
    IM_VERIFY(font_cfg->FontData != NULL && font_cfg->FontDataSize > 0);
...
//    IM_VERIFY(!Locked && "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!");
    ...
    if (!data)
    {
        IM_VERIFY_USER_ERROR(0, "Could not load font file!");
        return NULL;
    }
    ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
    if (font_cfg.Name[0] == '\0')
...
//    IM_VERIFY(!Locked && "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!");
    ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
    IM_VERIFY(font_cfg.FontData == NULL);
...
{
    IM_VERIFY(width > 0 && width <= 0xFFFF);
    IM_VERIFY(height > 0 && height <= 0xFFFF);
...
int ImFontAtlas::AddCustomRectFontGlyph(ImFont* font, ImWchar id, int width, int height, float advance_x, const ImVec2& offset)
{
    ...
    IM_VERIFY(id <= IM_UNICODE_CODEPOINT_MAX);
#endif
    IM_VERIFY(font != NULL);
    IM_VERIFY(width > 0 && width <= 0xFFFF);
    IM_VERIFY(height > 0 && height <= 0xFFFF);
...
{
    IM_VERIFY(TexWidth > 0 && TexHeight > 0);   // Font atlas needs to be built before we can calculate UV coordinates
    IM_VERIFY(rect->IsPacked());                // Make sure the rectangle has been packed
...
bool ImFontAtlas::GetMouseCursorTexData(ImGuiMouseCursor cursor_type, ImVec2* out_offset, ImVec2* out_size, ImVec2 out_uv_border[2], ImVec2 out_uv_fill[2])
{
    ...
    IM_VERIFY(PackIdMouseCursors != -1);
...
{
    IM_VERIFY(x >= 0 && x + w <= texture->TexWidth);
    IM_VERIFY(y >= 0 && y + h <= texture->TexHeight);
    IM_VERIFY(texture);
...
{
    IM_VERIFY(x >= 0);
    IM_VERIFY(x+width <= TexWidth);
    IM_VERIFY(y >= 0);
    IM_VERIFY(y+height <= TexHeight);
    IM_VERIFY(width > 0);
    IM_VERIFY(height > 0);
...
void ImGui::RenderMouseCursor(ImDrawList* draw_list, ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor, ImU32 col_fill, ImU32 col_border, ImU32 col_shadow)
{
    if (mouse_cursor == ImGuiMouseCursor_None)
        return;
    IM_VERIFY(mouse_cursor > ImGuiMouseCursor_None && mouse_cursor < ImGuiMouseCursor_COUNT);
...
static void stb__match(const unsigned char *data, unsigned int length)
{
    ...
    IM_VERIFY(stb__dout + length <= stb__barrier_out_e);
...
static unsigned int stb_decompress(unsigned char *output, const unsigned char *i, unsigned int /*length*/)
{
    ...
    for (;;) {
        ...
        if (i == old_i) {
            if (*i == 0x05 && i[1] == 0xfa) {
                IM_VERIFY(stb__dout == output + olen);
                if (stb__dout != output + olen) return 0;
                if (stb_adler32(1, output, olen) != (unsigned int) stb__in4(2))
                    return 0;
                return olen;
            } else {
                IM_VERIFY(0); /* NOTREACHED */
                return 0;
            }
        }
        IM_VERIFY(stb__dout <= output + olen);
